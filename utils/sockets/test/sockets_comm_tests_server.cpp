...
class Sockets_Comm_Test_Server : public ::testing::Test
{
    public:
        ...
        virtual void SetUp()
        {
            ASSERT_EQ(API_SUCCESS, sockets_init(BOOL_TRUE, SERVER_PORT, NUM_SOCKETS, NULL));

            printf(" Server socket initialized successfully at port %d.\n"
...
class Test_socket_socket_comm: public ::Sockets_Comm_Test_Server { };
TEST_F(Test_socket_socket_comm, duplex_comm)
{
    SOCKET_HDL server_sock[NUM_SOCKETS];
    UINT16_F i;
    ...
    {
        ASSERT_EQ(API_SUCCESS, socket_alloc(&server_sock[i]));
    }

    /* Client -> Server */
    ASSERT_NE(SOCKET_ERROR, socket_recv(server_sock[0], recv_buf, BUFSIZE, 0));
    EXPECT_EQ(0, pltfm_memcmp(recv_buf, send_buf[0], BUFSIZE));
    log_info((" Received sent message.\r\n"));

    /* Client -> Server */
    ASSERT_NE(SOCKET_ERROR, socket_recv(server_sock[1], recv_buf, BUFSIZE, 0));
    EXPECT_EQ(0, pltfm_memcmp(recv_buf, send_buf[2], BUFSIZE));
    log_info((" Received sent message.\r\n"));

    /* Server -> Client */
    ASSERT_NE(SOCKET_ERROR, socket_send(server_sock[1], send_buf[2], BUFSIZE, 0));
    log_info((" Sent message.\r\n"));

    /* Server -> Client */
    ASSERT_NE(SOCKET_ERROR, socket_send(server_sock[0], send_buf[1], BUFSIZE, 0));
    log_info((" Sent message.\r\n"));

    /* Server -> Client - Write 0 length */
    ASSERT_LE(socket_send(server_sock[1], send_buf[0], 0, 0), 0);
    log_socket_error();
    log_info((" Sent message.\r\n"));

    /* Server -> Client */
    ASSERT_GT(socket_send(server_sock[0], send_buf[0], BUFSIZE, 0), 0);
    log_info((" Sent message.\r\n"));

    for (i = 0; i < NUM_SOCKETS; i++)
    {
        ASSERT_EQ(API_SUCCESS, socket_free(server_sock[i]));
    }
}

class Test_socket_data_fidelity: public ::Sockets_Comm_Test_Server,
...
TEST_P(Test_socket_data_fidelity, random_write_read)
{
    ...
    log_info(("Waiting to receive confirmation.\n"));
    r_num = socket_recv(hdl, &r_conf, 1, 0);
    EXPECT_EQ(1, r_num);
    EXPECT_EQ(99, r_conf);

    /* 
     * Receive at server data from client.
     */
    do
    {
        do
     ...
     * Sending confirmation at the fag end to ensure that the next iteration of
     * the client doesn't startup before the server starts up.
     */
...
class Test_socket_file_fidelity: public ::Sockets_Comm_Test_Server,
...
TEST_P(Test_socket_file_fidelity, random_write_read)
{
    ...
    char w_file_name[100] = RCS_ROOT "test_sv_";
    ...
    log_info(("Sending confirmation for received data.\n"));
    r_conf = 99;
    s_num = socket_send(conf_hdl, &r_conf, 1, 0);
    EXPECT_EQ(1, s_num);
}
